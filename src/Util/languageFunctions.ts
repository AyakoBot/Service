...
export default (t: BaseLang) => ({
 ...
 getGuild: (guild: RGuild | APIPartialGuild | APIInviteGuild | APIWebhookSourceGuild) =>
 ...
 getInvite: (invite: RInvite | { code: string }) =>
 ...
 getIntegration: (integration: RIntegration | { id: string; name: string }) =>
 ...
 getWebhook: (webhook: RWebhook | { id: string; name: string | null }, type?: string) =>
 ...
 getSticker: (sticker: RSticker) => `**${t.t.Sticker()} \`${sticker.name}\` / \`${sticker.id}\`**`,
 getStageInstance(stageInstance: RStageInstance, channel: RChannel | null) {
