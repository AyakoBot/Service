...
test('returns root first then breadth-first dependencies', () => {
 ...
 const resolved = resolvePluginDependencies.call(client, ticketing as never);
 assert.deepEqual(
...
test('deduplicates diamond dependencies', () => {
 ...
 const names = resolvePluginDependencies.call(client, ticketing as never).map(
...
test('is cycle-safe', () => {
 ...
 const names = resolvePluginDependencies
  .call(client, a as never)
  .map((plugin) => plugin.settingName);
...
test('root with no dependencies resolves to itself', () => {
 ...
 assert.deepEqual(
  resolvePluginDependencies.call(client, afk as never).map((plugin) => plugin.settingName),
  ['afk'],
 );
});
...
test('throws on unknown dependency', () => {
 ...
 assert.throws(
  () => resolvePluginDependencies.call(client, afk as never),
