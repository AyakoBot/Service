...
export const getAPIRule = (rule: RAutomod) => ({
...
const getSetting = (rule: RAutomod, type: SettingNames) => {
 switch (type) {
...
const updateRule = async (rule: RAutomod, type: SettingNames) => {
 switch (type) {
  case SettingNames.Active: {
    ...
    trigger_metadata: getAPIRule(rule) && {
     presets: rule.triggerMetadata.presets.includes(
      ...
      ? rule.triggerMetadata.presets.filter(
         (o) => o !== RAutomodKeywordPresetType.Profanity,
        )
      : [...rule.triggerMetadata.presets, RAutomodKeywordPresetType.Profanity],
    },
   });
   ...
   const res = await rule.client.util.request.guilds.editAutoModerationRule(rule.guild, rule.id, {
    trigger_metadata: getAPIRule(rule) && {
     presets: rule.triggerMetadata.presets.includes(
      ...
      ? rule.triggerMetadata.presets.filter(
         (o) => o !== RAutomodKeywordPresetType.SexualContent,
        )
      : [
   ...
   const res = await rule.client.util.request.guilds.editAutoModerationRule(rule.guild, rule.id, {
    trigger_metadata: getAPIRule(rule) && {
     presets: rule.triggerMetadata.presets.includes(
      ...
      ? rule.triggerMetadata.presets.filter(
         (o) => o !== RAutomodKeywordPresetType.Slurs,
        )
      : [...rule.triggerMetadata.presets, RAutomodKeywordPresetType.Slurs],
    },
   });
