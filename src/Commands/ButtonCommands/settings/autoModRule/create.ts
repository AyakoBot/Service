...
const getEventType = (type: 'keyword' | 'mention' | 'spam' | 'preset' | 'member') => {
 switch (type) {
  ...
  default: {
   return RAutomodEventType.MessageSend;
  }
 }
};
...
const getTriggerType = (type: 'keyword' | 'mention' | 'spam' | 'preset' | 'member') => {
 switch (type) {
  case 'mention': {
   return RAutomodTriggerType.MentionSpam;
  }
  case 'spam': {
   return RAutomodTriggerType.Spam;
  }
  case 'preset': {
   return RAutomodTriggerType.KeywordPreset;
  }
  case 'member': {
   return 6 as never;
  }
  default: {
   return RAutomodTriggerType.Keyword;
  }
 }
};
