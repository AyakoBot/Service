...
export default async (
 ...
 if (!(cmd instanceof RMessage) && !cmd.inCachedGuild()) return;
 ...
 const getThread = async () => {
  ...
  if (cmd instanceof RMessage) return t;
    ...
    components: [
     {
      ...
      disabled:
       !canFinish ||
       !(cmd instanceof RMessage ? cmd.member?.permissions : cmd.memberPermissions)?.has(
