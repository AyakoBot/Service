...
const parseOptions = (
 c: RCommandOption | RCommand,
 parentWasSubCommandGroup = false,
): string[] => {
 if (!('options' in c)) return [];

 return (
  (c.options as RCommandOption[])?.map((c1) => {
   const isSubCommandGroup = c1.type === RCommandOptionType.SubcommandGroup;

   switch (c1.type) {
    case RCommandOptionType.Subcommand:
    case RCommandOptionType.SubcommandGroup: {
     return `${
