...
export default async function (
 ...
 if (!settings.active || settings.frozen || !settings.messageActive) return;

 const channelId = await parentOf.call(this.client, data.channel_id);
 const roleIds = data.member?.roles ?? [];

 if (isBlocked(settings, { channelId, userId: data.author.id, roleIds })) return;
 if (isBlocked(settings, { channelId: data.channel_id, userId: data.author.id, roleIds })) return;
