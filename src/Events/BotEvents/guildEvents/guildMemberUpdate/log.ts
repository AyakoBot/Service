...
export default async (oldMember: RMember, member: RMember) => {
 ...
 if (member.flags !== oldMember.flags) {
  const oldFlags = new RMemberFlagsBitField(oldMember.flags).toArray();
  const newFlags = new RMemberFlagsBitField(member.flags).toArray();
