...
export default async (
 msgToLog: RMessage | APIMessageSnapshot,
 isBulk: RMessage[] | boolean = false,
 originalMsg: false | RMessage = false,
) => {
 const msg = (originalMsg || msgToLog) as RMessage;
 ...
 const getDesc = () => {
  if (originalMsg && auditUser) {
   return lan.descDeleteForwardAudit(auditUser, msgToLog as APIMessageSnapshot, originalMsg);
  }
  if (originalMsg) {
   return lan.descDeleteForward(
    msgToLog as APIMessageSnapshot,
    originalMsg as RMessage,
   );
  }
