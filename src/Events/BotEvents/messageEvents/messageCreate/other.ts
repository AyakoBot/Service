...
const gvMessageCheck = (msg: RMessage) => {
  ...
  msg.client.util
   ...
   .then((m) => {
    ...
    jobs.scheduleJob(getPathFromError(new Error()), new Date(Date.now() + 10000), () => {
     if (m) msg.client.util.request.channels.deleteMessage(m as RMessage);
    });
   });
 };
...
const amMessageCheck = (msg: RMessage) => {
...
const amInproperStaffPingIdiot = async (msg: RMessage) => {
 if (msg.author.bot) return;
