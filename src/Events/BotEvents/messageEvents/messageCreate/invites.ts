...
export default async (msg: RMessage) => {
 ...
 if (settings.wlchannelid.some((c) => msg.channel.id === c)) return;
 if (settings.wlchannelid.some((c) => msg.channel.parentId === c)) return;
 ...
 if (
  msg.type !== RMessageType.AutoModerationAction &&
...
const isExternalInviteSource = async (invite: string, guild: Discord.Guild) => {
 ...
 if (inviteData.type !== RInviteType.Guild) return false;
