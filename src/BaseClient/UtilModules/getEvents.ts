...
// TODO: implement old redis events
...
export const getBotEvents = async () =>
 (await glob(`${eventPath}/BotEvents/**/*`))
  ...
  .filter((path) => {
   ...
   if (!Object.values(GatewayDispatchEvents).includes(eventName as never)) {
    return false;
   }
   return true;
  });
 ...
 * @returns {Promise<{ ProcessEvents: string[], RestEvents: string[], BotEvents: string[] }>}
