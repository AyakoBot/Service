 ...
 * @param color - A string, number or RMember object representing the color.
 * @returns A number representing the color.
 */
export default (
 color?: string | number | RMember | null | undefined | keyof typeof colors,
): number => {
 if (!color) return CT.Colors.Base;

 if (color instanceof RMember) {
  if (!color) return CT.Colors.Base;
