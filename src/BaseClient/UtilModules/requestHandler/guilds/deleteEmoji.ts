 ...
 */
export default async (guildId: string, emojiId: string, reason?: string) => {
 if (process.argv.includes('--silent')) return new Error('Silent mode enabled.');

 if (!(await canDeleteEmoji(guildId, (await getBotMemberFromGuild(guildId)).user_id))) {
 ...
 */
export const canDeleteEmoji = (guildId: string, userId: string) =>
