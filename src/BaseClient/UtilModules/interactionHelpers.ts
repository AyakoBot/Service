 ...
 */
const reply = async (
 cmd: Discord.ChatInputCommandInteraction | RMessage | Discord.ButtonInteraction,
 ...
 if (!desc || (con?.reqUser && !users.length)) {
  if (cmd instanceof RMessage) {
   const realCmd = (
    await getChannel.guildTextChannel((cmd as RMessage).channelId)
   )?.messages.cache.get((cmd as RMessage).id);
   if (!realCmd) return;
   ...
   Jobs.scheduleJob(getPathFromError(new Error()), new Date(Date.now() + 10000), async () => {
    ...
    if (await isDeleteable(m as RMessage)) {
     request.channels.deleteMessage(m as RMessage);
    }
    if (await isDeleteable(cmd as RMessage)) {
     request.channels.deleteMessage(cmd as RMessage);
    }
   });
 ...
 if (cmd instanceof RMessage) {
  ...
  if (
   (await isDeleteable(realCmd as RMessage)) &&
   cmd.type !== RMessageType.UserJoin
  ) {
   await request.channels.deleteMessage(realCmd as RMessage);
  }
  const content = String(payload.content);
  delete payload.content;

  const m = (await replyMsg(msg, payload, 'interactions')) as RMessage;
  if (m && (await isEditable(m))) request.channels.editMsg(m as RMessage, { content });
 } else {
  replyCmd(
   cmd,
...
const getDesc = <T extends keyof CT.Language['slashCommands']['interactions']>(
 ...
 lan: CT.Language['slashCommands']['interactions'][T],
 cmd: Discord.ChatInputCommandInteraction | Discord.ButtonInteraction | RMessage,
 legacyrp: boolean,
) => {
 ...
 */
const parseMsgUsers = async (msg: RMessage) => {
 ...
 if (mentionedUsers?.length) {
  ...
  const messages = new Array(mentionChunks.length)
   .fill(null)
   .map(() => structuredClone(msg)) as RMessage[];
