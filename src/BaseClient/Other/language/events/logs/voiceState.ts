...
export default (t: CT.Language) => ({
 ...
 descCreate: (user: RUser, channel: Discord.GuildChannel, channelType: string) =>
 ...
 descUpdate: (user: RUser, channel: Discord.VoiceBasedChannel, channelType: string) =>
  t.stp(t.JSON.events.logs.voiceState.descUpdate, {
   user: t.languageFunction.getUser(user),
   channel: t.languageFunction.getChannel(channel, channelType),
  }),
 descDelete: (user: RUser, channel: Discord.GuildChannel, channelType: string) =>
